...
contract DeliveryProvider is DeliveryProviderGovernance, IDeliveryProvider {
    ...
    function setAssetConversionBufferPub(
        uint16 targetChain,
        uint16 tolerance,
        uint16 toleranceDenominator
    ) public {
        setAssetConversionBuffer(targetChain, tolerance, toleranceDenominator);
    }

    function setPriceInfoPub(
      uint16 updateChainId,
      GasPrice updateGasPrice,
      WeiPrice updateNativeCurrencyPrice
    ) public {
    ...
    function setChainIdPub(uint16 thisChain) public {
      setChainId(thisChain);
    }

    function setMaximumBudgetPub(uint16 targetChain, Wei amount) public {
      setMaximumBudget(targetChain, amount);
    }

    function setChainSupportedPub(uint16 targetChain, bool isSupported) public {
      setChainSupported(targetChain, isSupported);
    }
