...
contract Storage {
    struct WormholeState {
        ...
        // Mapping of guardian_set_index => expiration timestamp
        mapping(uint32 => uint256) guardianSetsExpiration;
...
contract Setters is State {
    ...
    function expireGuardianSet(uint32 index) internal {
        _state.guardianSetsExpiration[index] = uint32(block.timestamp) + 86400;
    }

    function storeGuardianSet(uint8 parity, uint32 index, uint256 px, uint256 expiration) public {
        _state.guardianSets[index] = keccak256(abi.encodePacked(parity, px));
        _state.guardianSetsExpiration[index] = expiration;
    }
...
contract Getters is State {
    function getGuardianSet(uint32 index) public view returns (bytes32) {
        return _state.guardianSets[index];
    }

    function getGuardianSetsExpiration(uint32 index)
        public
        view
        returns (uint256)
    {
        return _state.guardianSetsExpiration[index];
    }
...
contract Messages is Getters {
    ...
    */
    function verifyVM(StructsCom.VM memory vm) public view returns (bool valid, string memory reason) {
        ...
        */
        if(guardianSet == bytes32(0)){
            return (false, "invalid guardian set");
        }

        /// @dev Checks if VM guardian set index matches the current index (unless the current set is expired).
        if(vm.guardianSetIndex != getCurrentGuardianSetIndex() && expirationTimestamp < block.timestamp){
    ...
    function verifySignatures(
        ...
        require(
            keccak256(abi.encodePacked(parity, px)) == guardianSet,
            "invalid guardianSet"
        );
        require(rAddress != address(0), "invalid address of R");
        uint256 c = uint256(
            keccak256(abi.encodePacked(rAddress, parity, px, hash))
        );

        uint256 spx = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141 -
                mulmod(
                    s,
                    px,
                    0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
                );
        uint256 cpx = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141 -
                mulmod(
                    c,
                    px,
                    0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
        ...
        require(spx != 0, "invalid value of s*px");

        valid =
            rAddress ==
            ecrecover(bytes32(spx), parity, bytes32(px), bytes32(cpx));
        reason = "";
    }
    ...
    function parseVM(bytes memory encodedVM) public pure virtual returns (StructsCom.VM memory vm) {
        ...
        // Parse Signatures
        vm.rAddress = address(uint160(uint256(encodedVM.toBytes32(index))));
        index += 32;
        vm.parity = encodedVM.toUint8(index);
        index += 1;
        vm.px = encodedVM.toUint256(index);
        index += 32;
        vm.s = encodedVM.toUint256(index);
        index += 32;
...
abstract contract Governance is GovernanceStructs, Messages, Setters, ERC1967Upgrade {
    ...
    function submitSetMessageFee(bytes memory _vm) public {
        ...
        // Verify the VAA is for this chainec
    ...
    function submitNewGuardianSet(bytes memory _vm) public {
        ...
        storeGuardianSet(upgrade.parity, upgrade.newGuardianSetIndex, upgrade.px, upgrade.expirationTime);
...
contract WormholeCom is Governance {
    ...
    function setChainIDs(uint16 newChainId, uint256 evmChainId) public {
        setChainId(newChainId);
        setEvmChainId(evmChainId);
    }

    function setGuardianSet(uint8 parity, uint32 index, uint256 px, uint256 expiration) public {
        expireGuardianSet(getCurrentGuardianSetIndex());
        storeGuardianSet(parity, index, px, expiration);
